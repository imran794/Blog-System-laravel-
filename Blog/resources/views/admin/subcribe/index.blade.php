        ...
        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
            <div class="card">
                <div class="header">
                    <h2>
                        All Post
                        <span class="badge bg-blue">{{ $subcribes->count() }}</span>
                    </h2>
                </div>
                <div class="body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover dataTable js-exportable">
                            ...
                            <tbody>
                                @foreach($subcribes as $key=>$subcribe)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $subcribe->email }}</td>
                                   
                                    <td>{{ $subcribe->created_at }}</td>
                                    <td>{{ $subcribe->updated_at }}</td>


                                    <td class="text-center">
                                        <a href="{{ route('admin.post.edit',$subcribe->id) }}" class="btn btn-info waves-effect">
                                            <i class="material-icons">edit</i>
                                        </a>
                                        <button class="btn btn-danger waves-effect" type="button" onclick="deletesubcribe({{ $subcribe->id }})">
                                            <i class="material-icons">delete</i>
                                        </button>
                                        <form id="delete-subcribe-{{ $subcribe->id }}" action="{{ route('admin.subcribe.destroy',$subcribe->id) }}" method="POST" style="display: none;">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
    ...
    function deletesubcribe(id) {
        swal({
            ...
            if (result.value) {
                ...
                document.getElementById('delete-subcribe-' + id).submit();
            } else if (
